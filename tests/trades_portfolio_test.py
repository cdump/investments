...
from investments.trades_fifo import analyze_portfolio
...
def test_analyze_portfolio_different_kinds():
    ...
    res = analyze_portfolio(request_trades)

    assert len(res) == 2
    res_dict = {x[0]: x[1] for x in res}
    assert res_dict[str(ticker_stock)] == 5
    assert res_dict[str(ticker_option)] == 10
...
analyze_portfolio_testdata = [
    ...
    ([('2018-01-01', 'FOO', 100500), ('2018-01-01', 'BAR', 1998), ('2020-10-12', 'FOO', 7.98), ('2018-01-01', 'BAR', -3)],
     [('BAR (Stock)', 1998 - 3), ('FOO (Stock)', 100500 + 7.98)]),
...
def test_analyze_portfolio(trades: list, expect_portfolio: dict):
    ...
    resp_portfolio = analyze_portfolio(request_trades)

    assert expect_portfolio == resp_portfolio
